...
try {
  for (const prefix of ['/', '/nested/example/']) {
    ...
    await page.addInitScript(()=>{
      ...
      window.requestAnimationFrame=callback=>{let id=request(time=>{pending.delete(id);callback(time);});pending.add(id);return id;};
      window.cancelAnimationFrame=id=>{pending.delete(id);cancel(id);};
      window.__rafPending=()=>pending.size;
    });
    page.on('pageerror', e => errors.push(e.message));
    page.on('response', r => { if (r.status() >= 400) missing.push(`${r.status()} ${r.url()}`); });
    page.on('request', r => { const url = new URL(r.url()); if (url.host === '127.0.0.1:4196' && !url.pathname.startsWith(prefix)) escaped.push(url.pathname); });
    const start = Date.now();
    await page.goto(`http://127.0.0.1:4196${prefix}wuhan/`);
    await page.waitForFunction(() => window.__wuhan?.ready, null, { timeout: 120000 });
    const loadMs = Date.now() - start;
    await page.waitForFunction(()=>window.__wuhan.getState().urbanReady&&window.__wuhan.getState().urban.surfacesReady,null,{timeout:120000});
    await page.waitForFunction(()=>window.__wuhan.getState().urban.buildingCount>0,null,{timeout:120000});
    await page.waitForTimeout(5000);
    const initial = await page.evaluate(() => window.__wuhan.getState());
    assert.equal(initial.rendererCount, 1);
    assert.ok(initial.triangles > 10000);
    assert.equal(initial.mainLoopCount,1);
    assert.equal(await page.evaluate(()=>window.__rafPending()),1);
    assert.equal(initial.urbanErrors.length,0);
    ...
    if (prefix === '/') {
      for (const id of ['confluence','guishan','jianghanguan','cbd','wuchang','luojia','guanggu','donghu','bridge-sequence','overview','bridge-yangtze-first','bridge-yingwuzhou','bridge-yangsigang','bridge-erqi','bridge-qingchuan','bridge-yangtze-second']) {
        await page.evaluate(id=>window.__wuhan.fly(id),id);
        await page.waitForFunction(() => !window.__wuhan.getState().flying);
        assert.equal((await page.evaluate(()=>window.__wuhan.getState())).region,id);
        await page.waitForTimeout(5500);
        const metrics=await page.evaluate(()=>window.__wuhan.getState());
        assert.deepEqual(metrics.urbanErrors,[]);
        results.push({view:id,metrics});
        await page.screenshot({ path: `probe/wuhan/phase2-${id}-day.png` });
        if (['guishan', 'luojia', 'moshan'].includes(id)) {
          await page.locator('[data-light="sunset"]').click();
          await page.screenshot({ path: `probe/wuhan/phase2-${id}-sunset.png` });
    ...
    await page.waitForFunction(() => !window.__wuhan.getState().flying);
    if (prefix === '/') await page.screenshot({ path: 'probe/wuhan/phase2-moshan-mesh.png' });
    await page.locator('#mesh-toggle').click();
    await page.locator('#labels-toggle').click();
    ...
    results.push({ prefix, loadMs, initial, hillSamples, bridgeLayers, errors, missing, escaped });
    await page.evaluate(() => window.__wuhan.dispose());
    assert.equal(await page.locator('canvas').count(), 0);
    await page.close();
  }
  const mobile = await browser.newPage({ viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true, deviceScaleFactor: 1 });
  await mobile.goto('http://127.0.0.1:4196/wuhan/');
  await mobile.waitForFunction(() => window.__wuhan?.ready, null, { timeout: 120000 });
  await mobile.waitForFunction(()=>window.__wuhan.getState().urban.buildingCount>0,null,{timeout:120000});
  await mobile.waitForTimeout(6000);
  const mobileMetrics=await mobile.evaluate(()=>window.__wuhan.getState());
  assert.deepEqual(mobileMetrics.urbanErrors,[]);
  results.push({view:'mobile-emulated',metrics:mobileMetrics,device:'390x844 desktop Chrome emulation, not a physical phone GPU'});
  assert.equal(await mobile.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
  await mobile.screenshot({ path: 'probe/wuhan/phase2-mobile.png' });
  await mobile.locator('#info-toggle').tap();
  await mobile.screenshot({ path: 'probe/wuhan/phase2-mobile-data.png' });
  ...
  for (const failure of ['version', 'checksum', 'missing']) {
    ...
    await page.goto('http://127.0.0.1:4196/wuhan/');
